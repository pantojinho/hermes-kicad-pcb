...
# hardware design document (PDF): every figure measured while it is built
pip install pymupdf
python skills/kicad-pcb/scripts/design_doc.py --sch board.kicad_sch --pcb board.kicad_pcb \
...
| `skills/kicad-pcb/examples/` | Design briefs of the two examples |
| `skills/kicad-pcb/scripts/make_report.py` | Lightweight REPORT.md (Markdown prints + BOM + ERC/DRC summary) |
| `skills/kicad-pcb/scripts/attach_easyeda_3d.py` | Attach LCSC/EasyEDA STEP models (with explicit offsets) and export a board STEP |
...
3D render | Schematic | 3D schematic (`pictorial.py`)
---|---|---
![LED board generated headless on Windows and Linux](assets/simple-led-3d.png) | ![LED board schematic](assets/simple-led-schematic.svg) | ![LED board 3D schematic](assets/simple-led-3d-schematic.png)

Its hardware design document — [simple-led-design-document.pdf](assets/simple-led-design-document.pdf),
17 pages, from [the design brief](skills/kicad-pcb/examples/simple_led_brief.md):

[![Simple LED board design document preview](assets/simple-led-design-document-preview.png)](assets/simple-led-design-document.pdf)
...
3D schematic of the same sheet, generated by `pictorial.py` from the KiCad files (no
EasyEDA involved): two-terminal parts are drawn as their 3D model between the leads,
ICs/connectors carry a translucent 3D overlay so pin names stay readable.

![ESP32 example 3D schematic](assets/esp32-3d-schematic.png)

Hardware design document — [esp32-design-document.pdf](assets/esp32-design-document.pdf),
18 pages, from [the design brief](skills/kicad-pcb/examples/esp32_brief.md). It states
what this demo board does **not** do (no programming path, no EN/BOOT circuit, no
input protection) instead of hiding it:

[![ESP32 example design document preview](assets/esp32-design-document-preview.png)](assets/esp32-design-document.pdf)
...
Linux; end-to-end headless). *Historical result: the board files and the script that
produced it are not in this repository, so it cannot be re-verified from here.*
...
| 3D models | Auto-attached per LCSC part | Official footprints ship aligned 3D models; LCSC STEP via `attach_easyeda_3d.py` needs explicit offsets |
...
EasyEDA's "3D schematic" view draws each part as a 3D picture. The skill produces
the same kind of figure from KiCad's own files with `pictorial.py` — see the ESP32
example above. The 3D pictures are KiCad renders of each footprint's 3D model, so the
schematic shows exactly the package that will be placed on the board.

This skill bridges **EasyEDA → KiCad**, one way: `import-pro` / `import-std` bring
PCB geometry into `.kicad_pcb`, and `lcsc Cxxxxx` brings an LCSC part's symbol,
footprint and 3D model in. An LCSC 3D model is authored for the LCSC footprint: when
you attach it to a different footprint with `attach_easyeda_3d.py`, pass its offset
and rotation (`--map REF=model.step@dx,dy,dz,rx,ry,rz`) and check the render — the
script refuses to replace the already-aligned model of an official KiCad footprint
unless `--replace-official` is given. LCSC part numbers and JLCPCB Basic/Extended
status must be checked on JLCPCB at order time; the skill never infers them.
